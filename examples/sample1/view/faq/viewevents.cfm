<h2>How do I directly call views as events?</h2>
<h4>And why would I?</h4>
<p>That's two questions! OK... First for the how...</p>
<h3>How do I directly call views as events?</h3>
...
<p><em>How about a more concrete example?</em> OK... let's take you through the anatomy of that view called as an event.</p>
<ul>
	<li>It first checks if there's an assignment (think alias) for that class. If there is, it changes the requested class for that assignment (more on this as the FAQs are filled out).</li>
	<li>If the class exists for the requested controller:
		<ul>
	...
	<li>If there was no assignment for the requested class, and the class does not exist:
		<ul>
...
<h3>Why would I directly call views as events?</h3>
<p>ColdFusion is all about simplicity. LightFront is all about simplicity. Not every page we're going to want to show on your site will be dynamic. For pages that are largely static, directly calling your view as an event makes a lot of sense and it's a lot less hassle than in other frameworks.</p>
<p>Rather than the framework calling a controller calling a view, LightFront checks to see if there's a valid controller for that class (&quot;about&quot;, in this example).</p>
...
<p>Mind you, you can always define events any way you want to. There's nothing stopping you if you want to build a CFC method for that event, and I doubt there's nothing wrong with that going forward. You could also call a switch. The benefit of calling views directly means less coding time, less code to create events, and a cleaner implementation. It's a pragmatic solution to </p>