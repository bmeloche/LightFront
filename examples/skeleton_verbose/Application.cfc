	...
	<cffunction name="setCustomLightFrontSettings" access="private" returntype="struct" hint="I set LightFront application settings.">
		<cfscript>
			...
			*	production = Application restarts and dumps are only provided on the server or by specific IP subset. Restricted mode for security purposes.*/
			...
			//lfront.eventVariable = "do"; //eg. index.cfm?do=home.welcome; "do" is the default. home.welcome is the event. home = class, welcome = method.
			//lfront.eventDelimiter = "."; //You could also set to ":".
			//lfront.defaultClass = "main"; //used if no classes are defined. Default is main.
			//lfront.defaultMethod = "default"; //used if no methods are defined. Default is default.
			//lfront.defaultEvent = lf.defaultClass & lf.eventDelimiter & lf.defaultMethod. />
			...
			//lfront.preEvent = "layout.header"; //I'm using this to display the header, but it can be used on any event that will always run before the requested event.
			//lfront.postEvent = "layout.footer"; //I'm using this to display the footer, but it can be used on any event that will always run before the requested event.
			...
			//lfront.switch.switchRoot = "/controller/switch/";
